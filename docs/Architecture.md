...
| **Cổng API** | `API Gateway` (`Kong`/`Traefik`) | Điểm truy cập backend duy nhất, quản lý API, xác thực, giới hạn tốc độ. |
| **Quản lý danh tính** | `Keycloak` (`OIDC`) | Xác thực (`SSO`, `OIDC`) và ủy quyền người dùng/dịch vụ. |
...
| **Hệ thống truyền tải sự kiện** | `Event Bus` (`Apache Kafka`) | Trái tim của kiến trúc hướng sự kiện, xử lý mọi luồng dữ liệu và sự kiện (IoT Data Stream, Event Topics). |
